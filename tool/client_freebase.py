...
def get_sqlite_cursor():
    if not hasattr(thread_local, "sql_client_facc1"):
        # cd database; ln -s ../../LLM_KGQA/database/cache_facc1/ ./
...
class FreebaseClient:
    ...
    def get_type_object_name(self, mid):
        ...
        try:
            ...
            print(f"Error in get_type_object_name: {res}")
            print(f"sparql_txt: {sparql_txt}")
            return ""
...
def test():
    fb = FreebaseClient(end_point="http://192.168.4.189:9501/sparql")
