...
class ESClient:
    ...
    def bulk_insert(self, actions: List[dict], thread_count=8, disable_tqdm=False):
        ...
        for idx, (success, info) in enumerate(
            tqdm(
                helpers.parallel_bulk(self.client, actions, thread_count=thread_count, chunk_size=1000),
                colour="green",
                dynamic_ncols=True,
                desc="ES bulk insert",
                total=len(actions),
    ...
    def delete_index(self, index):
        ...
        self.client.indices.refresh(index=index, ignore_unavailable=True)
    ...
    def list_indices(self, index="*"):
        return self.client.indices.get(index=index)

    def list_info_of_index(self, index):
        "statistic info"
